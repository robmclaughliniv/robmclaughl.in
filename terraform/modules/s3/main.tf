...
locals {
  # Use the input variable directly to control creation
  create_bucket = var.create_bucket
  # Since we only create or don't create, directly reference the created resource when needed.
  # Use try() with splat expression to safely access the potentially non-existent resource.
  bucket_id  = try(aws_s3_bucket.website[0].id, null)
  bucket_arn = try(aws_s3_bucket.website[0].arn, null)
}
...
resource "aws_s3_bucket_lifecycle_configuration" "website_lifecycle" {
  # Apply this configuration only if the bucket is created by this module
  count  = local.create_bucket ? 1 : 0
  bucket = local.bucket_id
